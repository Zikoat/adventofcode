...
let input2 = "one two three four rouf";
...
function isPassphraseValid(passphrase) {
	...
	for (var i = 0; i < passphrase.length; i++) {
		for (var j = i + 1; j < passphrase.length; j++) {
...
function amountOfValidPassphrases(input) {
	let passphrases = parseInput(input);
	let count = 0;
	passphrases.forEach(passphrase => {
		if (isPassphraseValid2(passphrase)) {
			count++;
		}
	});
	return count;
...
console.log(amountOfValidPassphrases(input));
...
function isPassphraseValid2(passphrase) {
	...
	for (var i = 0; i < passphrase.length; i++) {
		for (var j = i + 1; j < passphrase.length; j++) {
			...
			if (arraysEqual(
					passphrase[i].split("").sort(),
					passphrase[j].split("").sort()
				)) {
				return false;
			}
		}

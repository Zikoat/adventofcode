...
const testInputRaw3 = `123 328  51 64 
 45 64  387 23 
  6 98  215 314
*   +   *   +  `
...
let currentColumnm = 0;
console.log("starting loop")

const currentMathThing = []
const currentColumnMath = []

for (let [index, symbolOrSpace] of symbols.entries()) {
  // console.log("loop number ", index)

  const currentNumberArray = numbers.map(numberLine => nonNull(numberLine[index])).join("").trim();
...
function parseInput(input: string): {
  ...
  const numberLines = inputParsed.toSpliced(inputParsed.length - 1, Infinity).map(line => {
    return line.map(num => {
      assInt(num);
      return Number(num)
    })
  ...
  ass(symbolLineRaw)
  const symbolLine = symbolLineRaw.map(symbol => { ass(symbol === "*" || symbol === "+"); return symbol })
  ass(numberLines.every(numberLine => numberLine.length === symbolLine.length))
...
function getLineResult(input: string): number {
  ...
  for (let [index, symbol] of symbolLine.entries()) {
    ...
    if (symbol === "+") {
      ...
      const productLine = numberLine.reduce((prev, cur, _i, _arr) => prev * cur, 1);

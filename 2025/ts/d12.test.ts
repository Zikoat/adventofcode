...
import { assIsGiftMatrix, assmeq, canFitString, combinationsWithCheck, createAllTransmutations, createBoard, createDedupedTransmutations, flipGiftVertically, isInBounds, isValidBoard, placeGift, rotateGift90Right, someValidPlacements, stringToGift, stringToMatrix, transposeGift, visualizeBoard, wrapGift, wrapGiftString, type Board, type CombinationChecker, type Gift, type Gifts } from "./d12"
...
describe(isValidBoard, () => {
  ...
  test("placed x position outside of board should be invalid", () => {
    asseq(
      isValidBoard(
        placeGift(
          createBoard({ gifts: [[["#"]]], width: 1, height: 1 }),
          { type: 0, rotation: 0, x: 1, y: 0 }
        )
      ),
      false
...
describe(combinationsWithCheck, () => {
  ...
  test("should not visit children when a parent check is false", () => {
    const spy = mock<CombinationChecker>((combination) => (combination).every(shit => shit !== 0))
  ...
  test("should return false and still visit all leaf nodes when all leaf nodes return false", () => {
    const spy = mock<CombinationChecker>((combination) => combination[2] === undefined)
